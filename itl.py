...
def map_sat(num, version):
    if version == 2:
        if num == 77:
            return "---", "M08"
        if num < 66:
            satno, msgno = num % 11, num // 11
            return f"S{satno + 1:02d}", f"M{msgno + 1:02d}"
        if 82 <= num <= 84:
            return f"R{(num - 82) % 3 + 1:02d}", f"N{1:02d}"
        if 85 <= num <= 95:
            return f"S{num - 84:02d}", f"N{2:02d}"
        if 96 <= num <= 107:
            return f"R{(num - 96) % 3 + 1:02d}", f"N{(num - 96) // 3 + 3:02d}"
        if num == 108:
            return "---", "SSS"
        if num == 111:
            return "---", f"N{8:02d}"
        ...
        if num < 88:
            satno, msgno = num % 11, num // 11
            return f"S{satno + 1:02d}", f"M{msgno + 1:02d}"
        raise ValueError(f"Invalid satellite number {num} for version 1")
    raise AssertionError("Unexpected ITL version")
...
if __name__ == "__main__":
    ...
    for seq in sys.argv[1:]:
        try:
            print(f"Key: {seq} -> {MAP_PRS[seq]} (type: {MAP_PRS_TYPE[seq]})")
            s, m = map_sat(MAP_PRS[seq], 2)
            print(f"Map: {s} {m}")
        except KeyError:
            print(f"Key: {seq} no exact match")
            bseq = hex2bin(seq)
            for i, b in enumerate(BIN_PRS):
                diff = bitdiff(bseq, b)
                if diff < 10:
                    print(
                        f"but: {int(b, 2):024x} ({i % 128:03d}/{i // 128}) matched "
                        f"with {diff} bits difference"
                    )
                    break
